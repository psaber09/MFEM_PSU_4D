...
int main(int argc, char *argv[])
{
   ...
   */
   ...
   x.ProjectCoefficient(Q);
    ...
    //cout << "\n Initial || E_h - E ||_{L^2} = " << x.ComputeL2Error(Q) << '\n' << endl;
...
double E_exact(const Vector &x)
{
    ...
    if (dim == 2)
    {
        ...
        //E_out = x(0)*x(0)*(1. - x(0))*(1. - x(0))*x(1)*x(1)*(1. - x(1))*(1. - x(1));
    }
...
double Q_exact(const Vector &x)
{
    ...
    if (dim == 2)
    {
        Q_out = sin(6.28*x(0))*sin(6.28*x(1));
        //Q_out = 0.0;
    }
    ...
    if (dim == 4)
    {
        ...
        double pi_var = 6.28318530718;
        //Q_out = sin(pi_var*x(0))*sin(pi_var*x(1))*sin(pi_var*x(2))*sin(pi_var*x(3));
